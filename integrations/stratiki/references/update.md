...
2. Use `lastUpdate.gitHead` and `updatePreflight` from `stratiki_begin`, current
   Git status, and focused history or diffs to identify meaningful changes.
   Ignore generated Stratiki output and ignored paths.
3. When `updatePreflight.shouldSkip` is `true` and the user requested only a
   routine refresh, do not investigate or author files; call `stratiki_finish`
...
6. Rank affected areas by runtime importance, dependency centrality, public
   ...
   `stratiki_inspect_claims` with the active `runId` and that page. Inspect by
...
8. Before changing material factual prose, call `stratiki_resolve_claims` with
   ...
   `stratiki_finish` remove its Claims sidecar.
...
11. Call `stratiki_finish`; it removes the temporary plan and completes Claims
